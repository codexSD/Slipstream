...
**Deferred: spec §5 network-change handling (added during transfer-remediation, 2026-08-25).** `SlipstreamPeer.NetworkChanged` is raised on `NetworkInterface.NetworkAddressChanged`, but nothing subscribes to it. Spec §5's "tear down, rediscover, resume" is therefore not implemented: the control/transfer/media servers stay bound to the address chosen at `StartAsync`, and the peer becomes unreachable after a real network switch (e.g. WiFi → mobile hotspot) until the app is restarted. This is not a stub or a partial implementation — it is a genuine subsystem (tearing down bound sockets, re-running discovery, resuming in-flight transfers from persisted bitmaps) that needs to own reconnection state and user-visible status, which belongs with the UI layer, not `Slipstream.Core`'s transfer/media remediation scope. Tracked in `docs/superpowers/plans/2026-08-25-transfer-remediation-deviations.md`.
